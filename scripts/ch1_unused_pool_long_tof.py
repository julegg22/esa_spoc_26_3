"""Expand pair pool with LONG-TOF search for unused idE.

Previous unused-pool sweep had 10% feasibility at short TOF. Long-TOF
(15-60d) may unlock pairs that short-TOF couldn't.
"""
import sys
import time
import json
import numpy as np
import multiprocessing as mp
from pathlib import Path
sys.path.insert(0, '/home/julian/Projects/esa_spoc_26_3/src')
sys.path.insert(0, '/home/julian/Projects/esa_spoc_26_3/scripts')

from ch1_long_tof_test import try_long_tof_transfer, _init, evaluate_row
from ch1_production_sweep import hohmann_lower_bound
from esa_spoc_26.ch1_trajectory import LtlTrajectory

ROOT = "/home/julian/Projects/esa_spoc_26_3/reference/SpOC4/Challenge 1 Luna Tomato Logistics/"


def _solve_pair(args):
    """Long-TOF sweep for one (idE, idL)."""
    idE, idL = args
    from ch1_long_tof_test import _UDP
    udp = _UDP[0]
    best = None
    for tof_d in (8, 11, 15, 25, 40):
        for t0_val in np.linspace(0, 2 * np.pi, 4, endpoint=False):
            for ea_dep in np.linspace(0, 2 * np.pi, 4, endpoint=False):
                for ea_arr in np.linspace(0, 2 * np.pi, 4, endpoint=False):
                    res = try_long_tof_transfer(idE, idL, tof_d, t0_val,
                                                   ea_dep, ea_arr)
                    if res is not None and (best is None or res[0] > best[0]):
                        best = res
    return idE, idL, best


def main(K=3, n_workers=8,
          out_results="/home/julian/Projects/esa_spoc_26_3/runs/ch1/unused_long_tof_results.json"):
    udp = LtlTrajectory(ROOT)
    aE_arr = udp.earth_data[:, 0]
    aL_arr = udp.moon_data[:, 0]
    eL_arr = udp.moon_data[:, 1]

    bank_path = Path("/home/julian/Projects/esa_spoc_26_3/solutions/upload/trajectory.json")
    used_idE = set()
    used_idL = set()
    if bank_path.exists():
        bank = json.load(open(bank_path))
        dv = bank[0]["decisionVector"]
        for i in range(0, len(dv), 21):
            if dv[i] >= 0:
                used_idE.add(int(dv[i]))
                used_idL.add(int(dv[i + 1]))
    print(f"Bank: {len(used_idE)} idE, {len(used_idL)} idL", flush=True)

    unused_e = [i for i in range(400) if i not in used_idE]
    print(f"Unused idE: {len(unused_e)}", flush=True)

    # Top-K idL per unused idE, prefer unused idLs
    pairs = []
    for idE in unused_e:
        scores = []
        for idL in range(400):
            m = hohmann_lower_bound(aE_arr[idE], aL_arr[idL])
            m_adj = m * (1 + eL_arr[idL])
            if idL not in used_idL:
                m_adj *= 1.5
            scores.append((m_adj, idL))
        scores.sort(reverse=True)
        for _, idL in scores[:K]:
            pairs.append((idE, idL))
    print(f"Candidates: {len(pairs)} pairs", flush=True)
    print(f"Estimated wall: ~{len(pairs) * 50 / n_workers / 3600:.1f}h",
          flush=True)

    t_start = time.time()
    results = {}
    n_done = 0
    n_valid = 0
    with mp.Pool(n_workers, initializer=_init) as p:
        for idE, idL, best in p.imap_unordered(_solve_pair, pairs, chunksize=1):
            n_done += 1
            if best is not None:
                n_valid += 1
                results[(idE, idL)] = best
            if n_done % 10 == 0:
                wall = time.time() - t_start
                rate = n_done / wall if wall > 0 else 0
                eta = (len(pairs) - n_done) / rate if rate > 0 else 0
                top_m = sorted([v[0] for v in results.values()],
                                reverse=True)[:3]
                top_str = ",".join(f"{m:.0f}" for m in top_m)
                ue = len(set(k[0] for k in results.keys()))
                print(f"  [{n_done:4d}/{len(pairs)}] valid={n_valid} "
                      f"new_idE={ue} top=[{top_str}]kg "
                      f"wall={wall:.0f}s ETA={eta:.0f}s", flush=True)
            if n_done % 30 == 0:
                serializable = {f"{k[0]},{k[1]}": [v[0], list(v[1]), v[2]]
                                 for k, v in results.items()}
                Path(out_results).write_text(json.dumps(serializable))

    serializable = {f"{k[0]},{k[1]}": [v[0], list(v[1]), v[2]]
                     for k, v in results.items()}
    Path(out_results).write_text(json.dumps(serializable))
    wall = time.time() - t_start
    ue = len(set(k[0] for k in results.keys()))
    print(f"\nDone in {wall:.0f}s: {n_valid}/{n_done} valid, {ue} new idE",
          flush=True)


if __name__ == "__main__":
    K = int(sys.argv[1]) if len(sys.argv) > 1 else 3
    nw = int(sys.argv[2]) if len(sys.argv) > 2 else 8
    main(K=K, n_workers=nw)
