"""Apply long-TOF (15-60d) search to ALL banked transfers + bank improvements."""
import sys
import time
import json
import numpy as np
        ...
        for t0_val in np.linspace(0, 2 * np.pi, 4, endpoint=False):
            for ea_dep in np.linspace(0, 2 * np.pi, 4, endpoint=False):
                for ea_arr in np.linspace(0, 2 * np.pi, 4, endpoint=False):
                    res = try_long_tof_transfer(idE, idL, tof_d, t0_val, ea_dep, ea_arr)
...
def main(n_workers=8):
    ...
    print(f"{len(active)} active transfers; baseline {-udp.fitness(dv)[0]:.0f}kg",
          flush=True)
    t0 = time.time()
    polished = list(dv)
    n_imp = 0
    ...
    print(f"\n{n_imp}/{len(active)} improved, +{total_gain:.0f}kg in "
          f"{time.time() - t0:.0f}s", flush=True)
    if f_p > -udp.fitness(dv)[0] + 0.5:
        bank_path.write_text(json.dumps([{
            "decisionVector": [float(v) for v in polished],
            "problem": "trajectory",
            "challenge": "spoc-4-luna-tomato-logistics"}]))
        print(f"BANKED: {-udp.fitness(dv)[0]:.0f} → {f_p:.0f}kg", flush=True)
