...
def main(iters=600, seed=0, drop=0.25):
    ...
    if START == "bank":                              # continue climbing from the current banked best
        bx = np.asarray(json.load(open(f"{ROOT}/solutions/upload/matching-i.json"))[0]["decisionVector"], np.int8)
        ...
        if cur_val > best + 1e-6:
            ...
            if cur_val > BANK:
                ...
                if feas:
                    ...
                    json.dump({"decisionVector": x_out.tolist(), "problem": "matching-i",
                               "challenge": "spoc-4-luna-tomato-logistics"},
                              open(f"{ROOT}/cache/ch1_m1_lpbasin_best_s{seed}.json", "w"))
