...
drops below the banked 112.996 for the SAME order, the gap is SCHEDULE optimization, not
...
def main(budget=200000):
    # POSITIVE CONTROL: reconstructed bank must reproduce 112.996 feasible
    obj0, mk0, feas0 = evalx(enc0)
    print(f"S1 startup control: reconstructed bank makespan={mk0:.4f} feasible={feas0} "
          f"(expect 112.996) obj={obj0:.4f}", flush=True)
    if not feas0 or abs(mk0-112.996) > 0.5:
    ...
    while not es.stop():
        ...
        for s in sols:
            ...
            if feas and mk < best_mk:
                ...
                print(f"  [{n_eval}] NEW BEST makespan={mk:.4f} (bank 112.996, {mk-112.996:+.4f}) "
                      f"[{time.time()-t0:.0f}s]", flush=True)
        es.tell(sols, vals)
        if time.time()-last > 30:
            print(f"  [{n_eval}] best={best_mk:.4f} sigma={es.sigma:.3f} [{time.time()-t0:.0f}s]", flush=True)
            last = time.time()
    print(f"\n=== S1 DONE: best feasible makespan = {best_mk:.4f} vs bank 112.996 ({best_mk-112.996:+.4f}) "
          f"over {n_eval} evals ===", flush=True)
    if best_mk < 112.996 - 1e-3:
