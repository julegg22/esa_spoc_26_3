...
CKPT = f"{ROOT}/cache/ch2_giant_rare_beam_best.json"
kt = KTTSP(INST)
...
# RARITY: low-in-degree cities are the periphery the makespan-greedy beam strands (E-711 diagnostic).
INDEG = defaultdict(int)
for (_i, _j) in KEYS:
    INDEG[int(_j)] += 1
RARE = {c for c in cities if INDEG[c] < 40}
...
def main(W=60, K=18, rare_bonus=0.0, start=-1):
    starts = [cities[start]] if start >= 0 else cities[:8]
    print(f"[E-711 rare-beam] W={W} K={K} rare_bonus={rare_bonus}; {len(RARE)} rare/{NG}; {len(starts)} starts", flush=True)
    ...
    for depth in range(1, NG):
        ...
        for st in beam:
            ...
            for (j, arr, tof, is_exc) in cs:
                succ.append({"t": arr, "last": j, "vis": st["vis"] | {j},
                             ...
                             "rare": st["rare"] + (1 if j in RARE else 0)})
        if not pc_done:
            print(f"[E-711 rare-beam] positive control: depth1 expanded to {len(succ)} successors [{time.time()-t0:.0f}s]", flush=True)
            pc_done = True
        if not succ:
            print(f"[E-711 rare-beam] stranded at depth {depth} (no successor from {len(beam)} states)", flush=True)
            break
        # PRUNE key: makespan discounted by rare cities visited -> periphery-covering states survive
        succ.sort(key=lambda s: s["t"] - rare_bonus * s["rare"])
        ...
        if depth % 20 == 0 or depth < 5:
            bt = min(beam, key=lambda s: s["t"]); mr = max(s["rare"] for s in beam)
            print(f"  depth {depth+1}: |beam|={len(beam)} best_depth={best['depth']} min_t={bt['t']:.1f}d "
                  f"(d/leg {bt['t']/max(len(bt['path'])-1,1):.3f}) rare_max={mr}/{len(RARE)} [{time.time()-t0:.0f}s]", flush=True)
        if depth % 25 == 0:
...
if __name__ == "__main__":
    ...
    rb = float(sys.argv[3]) if len(sys.argv) > 3 else 10.0
