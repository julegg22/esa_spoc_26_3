"""Expand v7: cover remaining 204 unused Moon idLs with iL-matched per-idE.

v6 banked 32 of 42 favorable (A_favorable: aL>4.5e6, eL>0.4) idLs.
Remaining: 10 missed A + 85 B_moderate + 109 C_LMO_circ = 204 unused idLs.

For each unused idL, pick top-K unused idEs with smallest |iE - iL|. This
minimizes plane change cost. K=5 per idL gives ~1020 pairs at 12-eval
reduced grid = ~25 sec/pair wall, total 53 min.
"""
import sys
import time
import json
...
def main(K=5, n_workers=8,
          out_results="/home/julian/Projects/esa_spoc_26_3/runs/ch1/bcp_apogee_expand_v7_results.json"):
    ...
    # Per-idL: pick top-K unused idEs with smallest |iE - iL|
    pairs = []
    seen = set()
    for idL in unused_l:
        scores = [(abs(iE_arr[idE] - iL_arr[idL]), idE) for idE in unused_e]
        scores.sort()
        for _, idE in scores[:K]:
            ...
            if key not in seen:
                seen.add(key)
                pairs.append(key)
    print(f"Candidates: {len(pairs)} pairs ({len(unused_l)} idL × K={K})",
           flush=True)
