"""Expand v9: extended t_max (40d) + raan_l sweep for the 106 unmatched.

After v8: 294 transfers, 106 unused idE × 106 unused idL.
v8 found valid solutions for only 25 unused idE — the remaining 91 may need
longer t_max to catch perilune (default 20d cap rejects late closest-approach
trajectories) or different raan_l (Moon orbit RAAN — fixed at 0 in v6/v7/v8).

v9 extends t_max_d=40 and sweeps raan_l ∈ {0, π/2, π, 3π/2}.

Per-idE iL-matched, K=5 idLs each. 106 × 5 × 4 raan_l = 2120 pairs.
Grid expanded for raan_l: 4 raan_e × 3 ea_arr × 2 t2_d × 4 raan_l = 96 evals.
Heavy — at ~5 sec/pair wall (4x v8 grid, 2x t_max), 2120 × 5 / 8 = 22 min.
"""
import sys
import time
import json
...
def main(K=5, n_workers=8,
          out_results="/home/julian/Projects/esa_spoc_26_3/runs/ch1/bcp_apogee_expand_v9_results.json"):
    ...
    # Per-idE: top-K unused idL by smallest |iE - iL|
    pairs = []
    seen = set()
    for idE in unused_e:
