"""Guard-bank the E-586 large candidate into solutions/upload/large.json.
...
Run: PYTHONPATH=src OMP_NUM_THREADS=1 micromamba run -n spoc26 \
        python scripts/ch2_e586_bank.py
"""
...
CAND = "/tmp/ch2_large_cand.json"
BANK = "solutions/upload/large.json"
INST = glob.glob("reference/SpOC4/*Keplerian*/problems/hard.kttsp")[0]
EPS = 1e-6
...
def main():
    ...
    shutil.copy(BANK, f"/tmp/bank_bak/large_{ts}.json")
    shutil.copy(BANK, f"{BANK}.bak.e586")

    tmp = "/tmp/_e586_bankstep.json"
    ...
    print(f"=== BANKED large {b_mk:.4f} -> {f_mk:.4f} d (-{b_mk - f_mk:.4f}); "
          f"feasible={f_feas} viols={f_v}; backup .bak.e586 + /tmp/bank_bak/large_{ts}.json ===")
