...
def top_hops(kt, neigh_in, path, cset, topK):
    """Return up to topK (tof, j) cheapest feasible hops from path.cur."""
    feas = []
...
def beam_build(kt, neigh_in, nodes, start, B, topK, deadline):
    ...
    while beam:
        ...
        if not children:
            break
        # onward-degree stranding penalty: unvisited nodes with few unvisited
        # cheap neighbors are at risk -> prefer children that keep flexibility
        def score(c):
            return c.mk
