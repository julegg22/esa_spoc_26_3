...
from ch1_stm_corrector import stm_newton_back
...
def solve_pair(udp, idE, idL, idD, restarts, gen, seed0=0):
    ...
    bep = UDPBackEcc(udp, idE, idL)                                # raw instance: orbit elements for the STM polish
    ...
    for rs in range(restarts):
        ...
        if fb < 1.3e4:                                             # feasible regime: polish to the official window
            cands = [xb]
            # E-738 LEVER: STM corrector polishes the ~km CMA seed to the official 384m/1e-6 window
            # (the precision bare CMA+Nelder-Mead cannot thread for the circular captures)
            try:
                xc, info = stm_newton_back(xb, bep.aM, bep.eM, bep.iM, bep.aE, bep.eE, bep.iE, iters=40)
                if info.get("ok"):
                    cands.append(np.asarray(xc))
            except Exception:
                pass
            res = minimize(lambda z: prob.fitness(z)[0], xb, method="Nelder-Mead",   # original NM fallback
                           options={"maxiter": 400, "fatol": 1e-3})
            if prob.fitness(res.x)[0] < fb:
                cands.append(res.x)
            for xcand in cands:                                   # best official-valid mass across candidates
                ov = official_row(udp, idE, idL, idD, xcand)
                if ov is not None and (best is None or ov[2] > best[2]):
                    best = ov
    return best
...
def main():
    ...
    CKPT = f"{ROOT}/cache/ch1_stm_fleet_w{shard}of{nshard}.json"
    print(f"[E-738 STM-FLEET] restarts={restarts} gen={gen} dv_thresh={dv_thresh} shard={shard}/{nshard}", flush=True)
    ...
    print(f"[E-738 STM-FLEET] shard {shard}/{nshard}: {len(cand)} of {len(cand_all)} expensive "
    ...
    print(f"\n[E-738 STM-FLEET] DONE: {improved} pairs improved, +{total_gain:.0f} kg total [{time.time()-t0:.0f}s]", flush=True)
