"""Guard-bank the E-568 medium ultrafine-retime candidate into solutions/upload/medium.json.

Independently re-scores with the OFFICIAL kt.fitness; commits ONLY if feasible
AND strictly better than the current bank, after a backup, with a round-trip
re-read check. Never submits.

Run: PYTHONPATH=src OMP_NUM_THREADS=1 micromamba run -n spoc26 \
        python scripts/ch2_e568_bank.py
"""
...
CAND = "/tmp/ch2_medium_ultrafine_candidate.json"
...
def main():
    ...
    print(f"candidate: mk={c_mk:.4f} feasible={c_feas} viols={c_v} len={len(cand_dv)}")
    print(f"bank:      mk={b_mk:.4f} feasible={b_feas} viols={b_v} len={len(bank_dv)}")
    ...
    shutil.copy(BANK, f"{BANK}.bak.e568")

    tmp = "/tmp/_e568_bankstep.json"
    ...
    print(f"=== BANKED medium {b_mk:.4f} -> {f_mk:.4f} d (-{b_mk - f_mk:.4f}); "
          f"feasible={f_feas} viols={f_v}; backup .bak.e568 + /tmp/bank_bak/medium_{ts}.json ===")
