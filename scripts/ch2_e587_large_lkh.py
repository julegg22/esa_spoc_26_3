    ...
    for it in range(N_ITERS):
        ...
        for pi in range(len(cur_pieces)):
            ...
            if w is None or not w["feas"]:
                # revert this piece if walk broke
                cur_pieces[pi] = nodes
                full = [g for p in cur_pieces for g in p]
                w = walk_stats(kt, full)
                print(f"  [p{pi}] n={len(nodes)} forbid={nforb} REVERT "
                      f"(walk infeas) ({time.time()-tb:.0f}s)", flush=True)
            else:
                epoch = {full[k]: w["times"][k]
                         for k in range(len(w["times"]))}
                epoch[full[-1]] = w["times"][-1]
                print(f"  [p{pi}] n={len(nodes)} forbid={nforb} -> "
