...
**How to use.** A load-bearing conclusion (bank, floor, wall) lists its NEW
assumptions in frontmatter `assumes: [ID, …]`; effective validity = that set ∪
the branch's inherited set. When a row flips to `suspect`/`refuted`, run the
...
|----|---|---------------------------|-----------|-------------|------------------|
| `ENC-grid` | **L4** | A Ch2 makespan-optimal tour is reachable by retiming a fixed visit order on a uniform time-grid encoding. | E-759 (idle 100% order-forced, 0.0d retime-removable); C-036 epoch-shift trap | Ch2 retiming-based "floor/walled" verdicts (medium M1 null; small retime floor) | **RE-RUN** under window-indexed / exact-clock joint sequence search (E-760) — *backlog, weekly review* |
| `SOLVER-gtsp-exc` | L6 | GLKH/GTSP can enforce the hard ≤5-exception constraint. | E-746 (abuses exceptions → 75.2d infeasible, or strands when penalized) | "Ch2-small GTSP lever CLOSED" (E-746 verdict) | **RE-RUN** with the exact-DP+LNS engine that enforces ≤5 natively (E-760, in progress) — R2/R4 case |
